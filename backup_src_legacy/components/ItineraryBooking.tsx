...
const ItineraryBooking: React.FC<ItineraryBookingProps> = ({ 
  ...
  statusText = "DISPONIBILIDAD ABIERTA"
}) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const message = `Hola Franpisco Adventure! Me gustaría reservar un cupo para la expedición: ${tourName}. Mis datos son:\n\n- Nombre: ${formData.name}\n- Fecha aprox: ${formData.date}`;
    ...
    <div className="bg-slate-900 p-6 md:p-8 text-white shadow-2xl relative overflow-hidden group">
      ...
      <div className="relative z-10">
        ...
        <h3 className="text-xl md:text-2xl font-black uppercase mb-4 tracking-tight leading-none">
          Inicia tu <span className="text-brand-blue">Expedición</span>
        </h3>
        <p className="text-[11px] md:text-xs text-white/50 mb-10 leading-relaxed max-w-[280px]">
          Solicita el paquete técnico completo de la ruta y asegura tu cupo para la temporada.
        </p>
        
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <div className="flex flex-col gap-2.5">
            <label className="text-[9px] font-black text-white/30 uppercase tracking-[0.2em]">FECHA APROXIMADA</label>
          ...
          <div className="flex flex-col gap-2.5">
            <label className="text-[9px] font-black text-white/30 uppercase tracking-[0.2em]">NOMBRES Y APELLIDOS</label>
            <input 
              type="text" 
              value={formData.name}
